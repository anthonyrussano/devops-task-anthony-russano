...
- Named egress rules are defined for every intended outbound path from the app and DB instances (MySQL 3306, proxy 3128, SSM 443, S3 prefix list). The Terraform `aws_vpc_security_group_egress_rule` resources used here follow the provider v5 recommended style, and Terraform removes AWS's default "allow all outbound" rule when it creates a managed VPC security group. The route tables add a second PCI 1.3.2 control: private and isolated subnets have no direct internet route, so internet egress is impossible without traversing the proxy.
